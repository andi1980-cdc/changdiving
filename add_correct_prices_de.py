#!/usr/bin/env python3
"""
Script to add correct prices from English version to German courses
"""

import re

def add_correct_prices_to_german_courses():
    with open('de/courses/index.html', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Correct prices from English version
    correct_prices = [
        '3,950 THB',  # Try Dive
        '3,950 THB',  # Refresher
    ...
    if len(courses) == len(correct_prices):
        for i, (course, price) in enumerate(zip(courses, correct_prices)):
    ...
    print(f"Added correct prices to {len(correct_prices)} German courses!")

if __name__ == "__main__":
    add_correct_prices_to_german_courses()
