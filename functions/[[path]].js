...
const isLang = (p) => p === "/" || p.startsWith("/en/") || p.startsWith("/de/") || p.startsWith("/th/");

// EXAKTE 410-ZIELE (ohne Slash am Ende)
...
// WILDCARD/PREFIX-410 (alles darunter)
...
export async function onRequest(context) {
  ...
  // 1) Harte 410: exakte Treffer
  if (FORCE_GONE_EXACT.has(path)) {
    return new Response("Gone", { status: 410 });
  }
  // 1b) Harte 410: Prefix/Wildcard
  if (FORCE_GONE_PREFIX.some(pre => path.startsWith(pre))) {
    return new Response("Gone", { status: 410 });
  }

  // 2) Erst Pages machen lassen (_redirects + statische Dateien)
  const res = await context.next();

  // 3) Wenn 404 bleibt: außerhalb Sprach-/Assetpfade => 410
  if (res.status === 404 && !isLang(path) && !isAsset(path)) {
    return new Response("Gone", { status: 410 });
  }
