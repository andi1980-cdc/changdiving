(function () {
  ...
  function update() {
    ...
    if (depthStr === "" || !isFinite(depth) || depth < 0) {
      out.textContent =
        "Enter a depth for approximate absolute pressure, relative gas volume and density (vs surface; fixed amount of gas).";
      return;
    }
    ...
    out.textContent =
      ...
      ata.toFixed(2) +
      " bar absolute (ata)\n" +
      "Relative gas volume (surface = 1): ≈ " +
      relVol.toFixed(3) +
      "\n" +
      "Relative gas density (vs surface): ≈ " +
      ata.toFixed(2) +
      "×";
  }
...
(function () {
  ...
  function update() {
    ...
    if (!isFinite(depth1) || !isFinite(vol1) || vol1 <= 0) {
      outCmp.textContent =
        "Enter depth and a positive volume to see the comparison.";
      outSurf.textContent = "";
      return;
    }
    ...
    if (!isFinite(depth2)) {
      outCmp.textContent = "Enter a new depth to see volume at that depth.";
      outSurf.textContent =
        "At surface (0 m, 1 bar abs): ≈ " + vAtSurf.toFixed(2) + " L";
      return;
    }
